...
class ArtistProposal
  ...
  def new_artist_proposal
    keys = %i[id profile_id event_id call_id production_id user_id subcategory other_categories other
              form_id own register_date category format title short_description description duration photos links cache children phone]
    artist_proposal = Hash[keys.map { |sym| [sym, params[sym]] unless params[sym].nil? }.compact]
    artist_proposal[:id] ||= SecureRandom.uuid
    form.each_key { |field| artist_proposal[field] = params[field] unless params[field].nil? }
    artist_proposal
  end
...
class ArtistOwnProposal
  ...
  def new_own_proposal
    keys = %i[id profile_id event_id call_id subcategory other_categories other form_id register_date
              category format title short_description description duration photos links cache children phone user_id]
    proposal = Hash[keys.map { |sym| [sym, params[sym]] unless params[sym].nil? }.compact]
    form.each_key { |field| proposal[field] = params[field] unless params[field].nil? }
