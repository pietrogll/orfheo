...
class ProposalManager
  def self.for_artistproposal(owner_id, user_id, params)
    ...
    class << manager
      ...
      def delete
        ...
        unless proposal[:own].is_true? || proposal[:user_id] == user_id || admin?
          send_rejection_mail(proposal,
      ...
      def remove_activities(proposal)
        activities = Repos::Activities.get({ '$and': [{ participant_proposal_id: proposal[:id] },
  ...
  def self.for_spaceproposal(owner_id, user_id, params)
    ...
    class << manager
      ...
      def delete
        proposal = delete_proposal_and_ :remove_activities, :remove_pictures, :remove_space_from_order,
                                        :remove_participant_from_call
        unless proposal[:own].is_true? || proposal[:user_id] == user_id || admin?
          send_rejection_mail(proposal,
                              { title: proposal[:space_name] })
        end
        proposal
      end
      ...
      def remove_activities(proposal)
        activities = Repos::Activities.get({ '$and': [{ host_proposal_id: proposal[:id] },
                                                      { event_id: proposal[:event_id] }] })
        Actions::UserDeletesActivities.run activities, proposal[:event_id]
      end
    end
  ...
  def modify
    ...
    proposal = if params[:own].is_true?
                 own_proposal_lib_dictionary(db_key).new(params,
                                                         form).to_h
               else
                 proposal_lib_dictionary(db_key).new(
                   params, form
                 ).to_h
               end
  ...
  def select_deselect
    ...
    ApiStorage.repos(db_key).select_deselect(params[:id])
  end
  ...
  def is_whitelisted?
    whitelisted_emails = call[:whitelist].map do |allowed|
      allowed[:email]
    end
  ...
  def send_proposal_mail(payload, mail_type_key)
    ...
    payload = payload.merge({ organizer_mail: organizer[:email][:value], event_name: event[:name], event_id: event[:id],
