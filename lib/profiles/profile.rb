...
class Profile
  ...
  def check_fields(user_id, params)
    raise Pard::Invalid::Params if mandatory.any? { |field| params[field].blank? } ||
                                   %i[postal_code locality].any? { |field| params[:address][field].blank? } ||
  ...
  def relations_same(user_id, profile_id = false)
    ...
    if profile_id
      if user_profiles.blank?
        ...
        user_profiles.inject([]) do |relations, user_profile|
          profile_id == user_profile[:id] ? relations : relations.push(user_profile.slice(:id, :name, :color))
        end
      end
    elsif user_profiles.blank?
      []
    else
      user_profiles.inject([]) do |relations, user_profile|
        relations.push(user_profile.slice(:id, :name, :color))
      end
    end
  end
end
