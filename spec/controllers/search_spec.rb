...
describe 'SearchController', skip: 'Legacy Sinatra controller - needs migration to Rails request specs' do
  ...
  describe 'Suggest' do
    ...
    it 'suggests names and facets of profiles' do
      post suggest_route, { query: %w[city o], lang: 'es' }
      expect(parsed_response['status']).to eq('success')
      expect(parsed_response['items']).to eq([
                                               {
                                                 'id' => 'other-facet',
    ...
    xit 'allows query from production fields' do
      post suggest_route, { query: %w[music tit], lang: 'es' }
      expect(parsed_response['status']).to eq('success')
      expect(parsed_response['items']).to eq([
                                               {
                                                 'id' => 'title',
  ...
  describe 'load_results' do
    ...
    let(:pull_params_keys) do
      %w[n_pulled n_total db_position first_half_results n_results starting_db_position]
    end
  ...
  describe 'suggest_program' do
    it 'suggets the tags' do
      expect(Services::Search).to receive(:get_program_suggestions).once.with('es', event_id, ['my'],
                                                                              {}).and_return [{ text: 'my_title',
                                                                                                type: 'title' }]
