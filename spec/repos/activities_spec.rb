...
describe Repos::Activities do
  ...
  describe 'get with double condition' do
    it 'gets all the activities that respect the condition' do
      expect(Repos::Activities.get({ '$and': [{ host_proposal_id: 'host_proposal_id' },
                                              { event_id: event_id }] })).to eq [activity, otter_activity]
    end
  end
  ...
  describe 'reset' do
    it 'resets all the activities of the modified proposal' do
      ...
      expect(Repos::Activities.get_by_id(activity_id)).to eq(activity.merge({ title: 'title',
