...
describe Services::Suggest do
  ...
  let(:profile) do
    {
      id: profile_id,
      facets: %w[artist creative manager],
      name: profile_name,
  ...
  describe 'suggest profiles name' do
    ...
    xit 'returns a list with the profile names including text query' do
      profile_collection.indexes.each do |index|
        puts index
      end
      query = { name: 'prof' }

      results = Services::Suggest.profile_names db_key,
                                                query

      expected_results = [{ name: profile[:name] },
                          { name: o_profile[:name] }, { name: other_profile[:name] }]
      expect(results).to eq expected_results
    end
