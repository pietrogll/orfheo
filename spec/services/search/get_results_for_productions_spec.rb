...
describe 'Services::Search Productions' do
  ...
  before(:each) do
    ...
    Repos::Productions.class_variable_get(:@@collection).insert_many([production, otter_production, other_production,
                                                                      o_production])
  end

  describe 'get_results for productions' do
    SAMPLE_PRODUCTIONS_SIZE = 15
    PRODUCTIONCARD_WANTED_PARAMS = %i[id title format category tags main_picture profile_id children
                                      cache duration profile_name profile_color].freeze
    ...
    it 'returns results whose elements are all different ' do
      ...
      results_5, = Services::Search.get_results(pull_params, db_key)
    ...
    it 'returns production with main_picture if first_half_results is true' do
      results_1, = Services::Search.get_results(intit_pull_params, db_key)
