...
module MetaRepos
  # Only the fields specified here can be part of the saved objects of the collection
  def self.for(database)
    [

      [StructFromHash.for(:id, :text, :source, :holders), :tags],
      [
        StructFromHash.for(:id, :name, :description, :size, :tech_specs, :tech_poss, :floor, :height, :capacity,
                           :allowed_categories, :allowed_formats, :links, :photos, :space_id, :user_id), :ambients
      ],
