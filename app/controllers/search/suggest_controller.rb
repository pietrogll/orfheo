class Search::SuggestController < ApplicationController
  ...
  def check_lang!(lang)
    valid_langs = [:es, :en, :ca]
    raise Pard::Invalid.new('invalid_language') unless valid_langs.include?(lang.to_sym)
  end
  ...
  def get_filters(params_filters)
    ...
    raise Pard::Invalid::FilterParams unless params_filters.is_a?(Hash) && params_filters.values.all? { |selections| selections.is_a?(Array) }
  ...
  def check_params(params_array)
    ...
    raise Pard::Invalid::QueryParams unless params_array.is_a?(Array) && params_array.all? { |param| param.is_a?(String) }
    params_array
  end
  ...
  def add_suggestion(suggestions, text, type)
    ...
    unless suggestions.any? { |suggestion| suggestion[:text].downcase == I18n.transliterate(translation).downcase }
