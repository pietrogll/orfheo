...
class ProductionsController < ApplicationController
  ...
  def check_production_ownership!(production_id)
    raise Pard::Unexisting.new('production') unless Repos::Productions.exists?(production_id)
    ...
    raise Pard::Invalid.new('production_ownership') unless (owner_id == current_user_id || admin?)
    owner_id
  end
