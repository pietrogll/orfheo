...
Pard.Welcome = function (status, params) {
  status = status || 'outsider';
  Pard.UserStatus['status'] = status;

  var _header;
  if (status == 'outsider') {
    _header = Pard.Widgets.LoginHeader();
  } else {
    _header = Pard.Widgets.InsideHeader(false, true);
  }
  ...
  _whole.append(_header.render(), _main.render().addClass(status + '-main'), _footer.render().addClass('footer-' + status));
