  ...
  ns.Widgets.TableManager = function (the_event, forms, displayer) {
    ...
    return {
      ...
      modifyArtist: function (artist) {
        ...
        var _this = this;
        if (profile.proposals) profile.proposals.forEach(function (proposal) {
          _this._reloadProposalRow(proposal, 'artist', profile)
        });
        Object.keys(the_event.spaces).forEach(function (key) {
          var space = the_event.spaces[key].space;
          if (space.profile_id == profile.profile_id) _this._reloadProposalRow(space, 'space', profile);
        })
        _selectCatReload();
      },
      _reloadProposalRow: function (proposal, proposal_type, profile) {
        if (profile) {
        ...
        if (proposal.form_id && _dataTables[proposal.form_id]) {
      ...
      modifyArtistParam: function (params) {
        ...
        this._reloadProposalRow(_proposal, 'artist', profile)
      },
      modifySpaceParam: function (params) {
        var space = the_event.spaces[params.proposal_id].space;
        this._reloadProposalRow(space, 'space')
      }
    }
  }
