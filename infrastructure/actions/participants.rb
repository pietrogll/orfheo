...
module Actions
  ...
  class ModifiesParticipantForManager
    def self.run(params)
      ...
      Services::Participants.make_up_for_manager participant
    end
  end
  ...
  class RemoveParticipantFrom
    def self.call(proposal)
      ...
      MetaRepos::Participants.delete(proposal[:profile_id]) if MetaRepos::Participants.exists?(proposal[:profile_id]) && get_profile_activities(
        proposal[:event_id], proposal[:profile_id]
      ).empty?
    end

    def self.program(activity)
      Repos::Programs.remove_participant activity[:program_id], activity[:host_id] unless get_profile_activities(
        activity[:event_id], activity[:host_id]
      ).length.positive?
      return if get_profile_activities(activity[:event_id], activity[:participant_id]).length.positive?

      Repos::Programs.remove_participant activity[:program_id], activity[:participant_id]
      MetaRepos::Participants.delete(activity[:participant_id]) if MetaRepos::Participants.exists?(activity[:participant_id]) && get_profile_proposals(
        activity[:event_id], activity[:participant_id]
      ).empty?
    end
  ...
  class CheckParticipantName
    ...
    def self.is_my_name?(participant_name, participant_id = nil)
      if participant_id.nil? || !(MetaRepos::Participants.exists?(participant_id) || Repos::Profiles.exists?(participant_id))
        return false
      end
