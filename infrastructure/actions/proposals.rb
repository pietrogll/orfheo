...
module Actions
  class UserGetsEventProfileProposals
    def self.run(event_id, profile_id)
      Repos::Spaceproposals.get({ '$and': [{ profile_id: profile_id },
                                           { event_id: event_id }] }) + Repos::Artistproposals.get({ '$and': [{ profile_id: profile_id },
                                                                                                              { event_id: event_id }] })
    end
  end
  ...
  class UserDeletesArtistProposal
    def self.run(owner_id, user_id, params)
      ...
      proposal[:own].is_true? ? "#{proposal[:profile_id]}-own" : proposal[:profile_id]
    end
