...
module Actions
  class UserCreatesEvent
    def self.run(user_id, params, is_admin = false)
      ...
      if params[:slug] && !params[:slug].to_s.strip.empty? && !Repos::Events.available_slug?(params[:slug])
        raise Pard::Invalid::ExistingSlug
      end

      event_instance = Event.new(user_id, params)
      if is_admin && params[:professional].is_true?
  ...
  class UserUpdatePartners
    def self.run(params)
      unless params[:partners].blank?
        partners = params[:partners].transform_values do |v|
          Util.arrayify_hash v
        end
      end
  ...
  class UserSetsEventSlug
    def self.run(user_id, event_id, slug)
      ...
      raise Pard::Invalid, 'invalid_slug' unless slug.size >= 3 && !(slug =~ /^[a-z0-9_-]*$/).nil?
      raise Pard::Invalid, 'slug_in_use' unless event[:slug].blank?
      raise Pard::Invalid, 'existing_slug' unless Repos::Events.available_slug? slug
