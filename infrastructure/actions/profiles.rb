...
module Actions
  class FilterProfile
    def self.run(profile)
      profile.delete(:phone) if profile.key?(:phone) && (profile[:phone].blank? || profile[:phone][:visible].is_false?)
      profile.delete(:email) if profile.key?(:email) && (profile[:email].blank? || profile[:email][:visible].is_false?)
      profile
    end
  end
  ...
  class UserCreatesProfile
    ...
    def self.update_relations(user_id, profile_id)
      ...
      user_profiles.each do |profile|
        ...
        profile[:relations] = user_profiles.inject([]) do |relations, user_profile|
  ...
  class UserDeletesProfile
    ...
    def self.delete_productions(profile_id)
      Repos::Productions.get_profile_productions(profile_id).each do |production|
