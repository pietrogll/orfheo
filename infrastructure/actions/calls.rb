...
module Actions
  ...
  class UserDeletesCall
    def self.run(call_id)
      ...
      unless call[:participants].blank? || !Repos::Events.is_future_event?(event[:id])
        send_email_to_participants call[:participants],
  ...
  class UserGetsCallProposals
    def self.run(call_id, type, filters = {})
      ...
      proposalsGetter = {
        'space' => -> { Repos::Spaceproposals.get(filters) },
        'artist' => -> { Repos::Artistproposals.get(filters) }
      }
