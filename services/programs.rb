    ...
    class << self
      ...
      def make_up_program_with(activities, program, artist_proposals, space_proposals, for_manager = true)
        ...
        activities.inject([]) do |program, activity|
          ...
          program + activity_splitted
        end
      end

      def split_activity_by_dateTime(activity, for_manager = true)
        Util.arrayify_hash(activity[:dateTime]).each_with_object([]) do |dt, activity_splitted|
      ...
      def arrange_ids(activities)
        activities.map do |activity|
          activity[:host_id] += '-own' if activity[:host_id].present? && !Repos::Profiles.exists?(activity[:host_id])
          activity[:participant_id] += '-own' if activity[:participant_id].present? && !Repos::Profiles.exists?(activity[:participant_id])
      ...
      def add_artist_proposal_info(artist_proposals, activity)
        unless artist_proposals.blank?
          ...
          unless artist_proposal.blank?
            %i[title short_description children].each do |k|
            ...
            if activity[:participant_subcategory].blank?
              activity[:participant_subcategory] =
                artist_proposal[:subcategory]
            end
          end
        end
        activity
