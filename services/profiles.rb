  ...
  class Profiles
    class << self
      ...
      def update_participations(profile_id)
        return unless participate?(profile_id) || is_organizer?(profile_id)
      ...
      def my_program(profile_id, is_future)
        ...
        my_activities_sorted_by_event_id = my_activities.each_with_object({}) do |activity, activities_obj|
          ...
          if activities_obj[activity[:event_id]].nil?
            ...
            unless activities_obj[activity[:event_id]].include? activity
              activities_obj[activity[:event_id]].push(activity)
            end
          end
        end
      ...
      def participate?(profile_id)
        return true if Repos::Calls.all.any? do |call|
