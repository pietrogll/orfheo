...
module ApiStorage
  class QueriesStore
    ...
    def build_profiles_query(params)
      params[:first_half_results].is_true? ? { "profile_picture.0": { '$exists': true } } : { "profile_picture.0": { '$exists': false } }
    end

    def build_productions_query(params)
      if params[:first_half_results].is_true?
        { '$and': [{ "main_picture.0": { '$exists': true } }, { main_picture: { '$size': 1 } }] }
      else
        { '$or': [{ "main_picture.0": { '$exists': false } },
                  { '$and': [{ main_picture: { '$type': 'array' } }, { main_picture: { '$size': 0 } }] }] }
      end
    end
    ...
    def build_spaces_query(params)
      if params[:first_half_results].is_true?
        ...
        { '$or': [{ "main_picture.0": { '$exists': false } },
                  { '$and': [{ "main_picture.0": { '$exists': true } }, { main_picture: { '$size': 0 } }] }] }
      end
    end

    def build_events_query(params)
      now = Time.now.to_i * 1000
      params[:first_half_results].is_true? ? { 'eventTime.time': { '$gte': now.to_s } } : { 'eventTime.time': { "$not": { "$elemMatch": { '$gte': now.to_s } } } }
    end
