  ...
  class Search
    ...
    class DbPuller
      ...
      def pull(n_wanted = N_WANTED)
        ...
        results = @queries_performer.execute(@pull_params[:db_position], n_wanted,
                                             @queries_performer.build(@pull_params))
      ...
      def new_starting_db_position(query)
        ...
        n_results.zero? ? n_results : @queries_performer.new_starting_db_position(n_results)
      end
      ...
      def n_remainder_from_results
        ...
        distance_from_starting_pos.positive? ? distance_from_starting_pos : @pull_params[:n_results] + distance_from_starting_pos
      end
    end
  end
end
