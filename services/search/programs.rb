...
module Services
  class Search
    class << self
      ...
      def check_lang!(lang)
        raise Pard::Invalid, 'invalid_language' unless %w[en es ca].include? lang

        @lang = lang.to_sym
      end

      def select_now(results, time)
        results.select do |performance|
      ...
      def query_program(program, tags, filters)
        program = filter_participants program, filters[:participants] if filters.key? :participants
        program = filter_hosts program, filters[:hosts] if filters.key? :hosts
        program = filter_other program, filters[:other] if filters.key? :other
        return program if tags.all?(&:blank?)

        program.select do |performance|
      ...
      def check_performance(performance, tag)
        ...
        searcheable_fields.any? do |field|
      ...
      def get_suggestions_for(matched_performances, query)
        ...
        matched_performances.each_with_object([]) do |performance, suggestions|
      ...
      def add_suggestions(suggestions, performance, query)
        if queriable? performance[:participant_name],
                      query
          add_suggestion(suggestions, performance[:participant_name], 'name',
                         'artist')
        end
        add_suggestion(suggestions, performance[:host_name], 'name', 'space') if queriable? performance[:host_name],
                                                                                            query
        add_suggestion(suggestions, performance[:title], 'title') if queriable? performance[:title], query
        if queriable? performance[:address][:locality],
                      query
          add_suggestion(suggestions, Util.string_keyed_hash_to_symbolized(performance[:address])[:locality],
                         'city')
        end
      end
      ...
      def add_suggestion(suggestions, text, type, icon = nil)
        ...
        unless suggestions.any? do |suggestion|
          suggestion[:text].downcase == I18n.transliterate(translation).downcase
        end
          suggestions.push({ id: translation, text: translation, type: type,
                             icon: icon })
        end
      end
      ...
      def translate(text)
        ...
        return dictionary[@lang][text.to_sym] if dictionary[@lang].key? text.to_sym
      ...
      def order_results(results)
        results.sort_by do |performance|
          [performance[:date], performance[:permanent] == 'false' ? 0 : 1, performance[:time].first,
           performance[:time].last]
        end
      end
    end
  end
