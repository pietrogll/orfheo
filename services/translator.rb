...
module Services
  class Translator
    ...
    def translate(key)
      key.to_s.split('.').inject(@dictionary) do |text_translated, key_token|
        text_translated = if text_translated.is_a? Hash
                            text_translated[key_token.to_sym]
                          else
                            text_translated[key.to_sym]
                          end
        text_translated
      end
    end
